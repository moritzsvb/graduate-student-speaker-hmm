...
\begin{frame}{What one usually does in the Hidden Markov case}
	...
	\pause
	\item Remember: $n^{-1}\sum_{k=0}^{n-1} g(Y_{k-m+1},\ldots, Y_{k}) \to \E[g(Y_0, \ldots, Y_{m-1})]$ almost surely,
	\item Have to always include the same amount of $m$ observations (approximation depth). 
	\pause
	\item There is a term $\ell(\theta)$ independent of the approximation depth (corresponding to $m = \infty$). Show now:
	$n^{-1} \sum_{k=0}^{n-1} \log \rho_\theta(Y_k | Y_{k-m+1}^{k-1}) \longrightarrow \ell(\theta) + \varepsilon.$
	\end{itemize}
\end{frame}

\begin{frame}{What we need to do in the \textbf{locally stationary} case}
	\small
		\begin{align*}
		n^{-1} \sum_{k=0}^{n-1} \log \rho_\theta(Y_k | Y_{k-m+1}^{k-1}) \longrightarrow \ell(\theta) + \varepsilon
	\end{align*}
...
\end{theoremblock}
\pause
Exact consistency depends on identifiability -- but this is a property of the stationary distributions. 
\end{frame}
...
\begin{frame}{The Doeblin condition and possible examples}
	\begin{itemize}
		...
		\item But even in simple finite state space models, Doeblin minorization is often not fulfilled: \small $\mathcal{X} = \{1,2,3\}$, \[q_{\theta}(i,j) = \begin{cases} \theta &\text{ if } j = i+1 \mod 3 \\ 1 - \theta &\text{ if } j = i-1  \mod 3 \\ 0 & \text{ else.} \end{cases}\]		
		\pause 
...
\begin{frame}{Potential modifications and the AR(1) case}
	\begin{itemize}
		...
		\item But at the end, all the \textit{classical} results rely on Doeblin-conditions at \textit{some place}. \pause
		\item \[
		...
		\item Quite intricate usage of ergodic theorem, seems to intentionally circumvent invoking (stronger) forgetting properties. 
	\end{itemize}
\end{frame}
...
\begin{frame}{All is not lost!}
	...
	\vspace{-0.3cm}
	\begin{align*}
	& \log \rho_{\alpha}^\nu(Y_{0,N}^{k,N}) = \log \rho_{\alpha}^\nu(\widetilde{Y_{0}^k}(t)) \\  & \qquad \qquad +  \underbrace{\left\{\log \rho_{\alpha}^\nu(Y_{0,N}^{k,N}) - \log \rho_{\alpha}^\nu(\widetilde{Y_{0}^k}(t))\right\}}_{=:d}
	\end{align*}
	\vspace{-0.3cm}
	\pause
	\begin{itemize}
		\item The first term converges by the stationary case.
		\item Under the right assumptions, $d$ vanishes in $L^p$ by interpolation arguments.
	\end{itemize}
\end{frame}
...
\begin{frame}{More is possible! But be vigilant!}
	\begin{itemize}
		\item Actually, in classical (Doeblin-) cases $\sup_{u \in [\frac{b}{2}, 1- \frac{b}{2}]} \norm{\hat{\theta}_n(u) - \theta^\ast(u)} \overset{P}{\to} 0$.
		\item Asymptotic normality of the MLE also possible in the first case.
		\pause
		\item Always keep the actual meaning of our asymptotics in mind. And the assumptions.
		\item Classical Hidden Markov model challenges remain important (especially identifiability of parameters)
	\end{itemize}
	\pause
	\vspace{1cm}

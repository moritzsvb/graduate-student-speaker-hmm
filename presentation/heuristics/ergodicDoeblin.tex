...
\begin{frame}{What one usually does in the Hidden Markov case}
	...
	\item $\tau^k(Y_0, Y_1, \ldots) = (Y_{k}, Y_{k+1}, \ldots)$
	\item Have to always include the same amount of $m$ observations. \begin{align*} g_k(Y_0^k) &= \log \rho_\theta(Y_k|Y_0^{k-1}) \approx \log \rho_\theta(Y_{k}|Y_{k-m+1}^{k-1}) \\ &= g(\tau^{k-m+1}(Y_0, Y_1, \ldots))\end{align*}
	\pause
	\item There is a term $\ell(\theta)$ independent of the approximation depth. Show now:

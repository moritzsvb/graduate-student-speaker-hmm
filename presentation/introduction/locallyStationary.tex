\begin{frame}{Structure}
	\begin{itemize}
		...
		\item The AR(1) process and $L^p$-local stationarity. 
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\pause
	\item In the case of ML estimation for iid data or homogeneous Markov time series we consider the estimators behavior as more data is added. 
	\item I.\,e. $\hat{\theta}_n \in \operatorname{argmax}_{\theta \in \Theta} \log \rho_\theta(Y_0^n)$, $n \to \infty$. 
	\pause
...
\begin{frame}{Developing an adequate asymptotic}
	...
	\normalsize
	\begin{itemize}
		\item Good would be an asymptotic $N \to \infty$, $b \to 0$ but still $bN \to \infty$. 
		\item Effectively, we then estimate a parameter at a point $t \in [0,1]$ in \textit{unit time}. 
		\item We get \textbf{both} more stationary around every $t \in [0,1]$ and gather more observations as $N \to \infty$
		\pause 
	\end{itemize}
\small
